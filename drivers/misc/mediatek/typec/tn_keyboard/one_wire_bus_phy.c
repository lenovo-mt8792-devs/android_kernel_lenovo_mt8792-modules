...
const static unsigned char uart_map_table[8][16] = {
	{ 0xE8, 0x39, 0x10, 0xBD, 0x3A, 0xA5, 0xC0, 0xA9, 0xD3, 0x35, 0xE0,
	  0xEB, 0xB0, 0x19, 0xED, 0x1F },
	{ 0xD1, 0xD5, 0xE6, 0xBC, 0xD4, 0xCC, 0xF9, 0xCE, 0xD0, 0xFB, 0xE5,
	  0xA1, 0xB1, 0xFD, 0x13, 0x1E },
	{ 0xC6, 0xCA, 0xA4, 0x11, 0xF6, 0xBF, 0xEE, 0xDC, 0xD2, 0xDA, 0xF3,
	  0xA2, 0xCB, 0x1C, 0xC8, 0x18 },
	{ 0x33, 0x3F, 0xE3, 0xAB, 0x38, 0xBB, 0xC3, 0xD8, 0xD6, 0x12, 0xB3,
	  0x30, 0xE2, 0x32, 0xA0, 0x1D },
	{ 0xDE, 0xAC, 0xE9, 0xAE, 0xF1, 0xBA, 0xC4, 0xE7, 0xCF, 0xEC, 0xEA,
	  0xB4, 0xF4, 0xFE, 0x14, 0xB2 },
	{ 0x3D, 0x3B, 0xDD, 0xB7, 0xEF, 0xB8, 0xC5, 0xCD, 0xAA, 0xDF, 0xBE,
	  0x37, 0xF5, 0xF8, 0xAF, 0x3C },
	{ 0x34, 0x3E, 0xA7, 0xAD, 0xB5, 0x16, 0x1A, 0xFC, 0xE4, 0xB6, 0xFF,
	  0xC2, 0x36, 0xD7, 0xC9, 0x1B },
	{ 0x17, 0xF0, 0xFA, 0xA8, 0xA3, 0xB9, 0xC7, 0xF2, 0x31, 0xD9, 0xE1,
	  0x15, 0xF7, 0xDB, 0xA6, 0xC1 },
};
				 ...
				 unsigned char *p_out, unsigned short in_len)
{
	...
	if (serial < 8) {
		for (i = 0; i < in_len; i++) {
			value = p_in[i];
			if (ONE_WIRE_BUS_PACKET_START_FLAG ==
			    OWB_LX_PACKET_START_FLAG) {
				p_out[i * 2] =
					uart_map_table[serial]
						      [0x0f -
						       ((value >> 4) & 0x0f)];
				p_out[i * 2 + 1] =
					uart_map_table[serial]
				...
				p_out[i * 2] =
					uart_map_table[serial]
						      [(value >> 4) & 0x0f];
				p_out[i * 2 + 1] =
					uart_map_table[serial][value & 0x0f];
			}
		}
		return true;
	} else {
				  ...
				  unsigned char dec_ser, unsigned char *p_out)
{
	...
	for (i = 0; i < 16; i++) {
		if (uart_map_table[dec_ser][i] == val_h) {
			val = ((ONE_WIRE_BUS_PACKET_START_FLAG ==
				OWB_LX_PACKET_START_FLAG) ?
				       15 - i :
				       i);
			status = 0x01;
			       ...
			       unsigned short in_len, unsigned char dec_ser)
{
	...
	if (dec_ser < 8) {
		for (i = 0; i < in_len; i++) {
			f_byte = one_wire_bus_decode_one_byte(p_in[i * 2],
							      p_in[i * 2 + 1],
							      dec_ser,
							      &out_data);
			if (f_byte == true) {
				       ...
				       unsigned short packet_max_len)
{
	...
	one_wire_bus_data_transcode(serial, &sum, &p_packet_buf[len * 2 + 12],
				    1);
