...
static const unsigned char tn_keyboard[KEYBOARD_NUM_KEYS] = {
	...
	136, 113, 115, 114, unk, unk, unk, 121, unk, 89,  93,  124, 92,	 94,
	95,  unk, unk, unk, 122, 123, 90,  91,	85,  unk, unk, unk, unk, unk,
	unk, unk, 111, unk, unk, unk, unk, unk, unk, unk, unk, unk, unk, unk,
	unk, unk, unk, unk, unk, unk, unk, unk, unk, unk, unk, unk, unk, unk,
	179, 180, unk, unk, unk, unk, unk, unk, unk, unk, unk, unk, unk, unk,
	unk, unk, unk, unk, unk, unk, unk, unk, unk, unk, unk, unk, unk, unk,
	unk, unk, unk, unk, unk, unk, 111, unk, unk, unk, unk, unk, unk, unk,
...
static int tn_keyboard_event_hander(struct input_dev *dev, unsigned int type,
...
int tn_keyboard_input_init(void)
{
	...
	for (i = 0; i < KEYBOARD_MM_NUM_KEYS; i++) {
		if (mm_tn_keyboard[i][1] != 0 && mm_tn_keyboard[i][1] != unk) {
			__set_bit(mm_tn_keyboard[i][1],
				  tn_keyboard_input->keybit);
			__set_bit(mm_tn_keyboard[i][0],
				  tn_keyboard_input->mscbit);
		}
	}
...
int tn_keyboard_input_report(char *buf)
{
	...
	for (i = 0; i < 8; i++) {
		input_report_key(input_dev, tn_keyboard[i + 224],
				 (kbd->new[1] >> i) & 1);
	...
	for (i = 2; i < 8; i++) {
		...
		if (kbd->new[i] > 3 &&
			...
			if (tn_keyboard[kbd->new[i]]) {
				input_report_key(input_dev,
						 tn_keyboard[kbd->new[i]], 1);
				//tn_keyboard_input_power_key_report();

				kb_debug("%s %d key int:%d hex:0x%x \n",
					 __func__, __LINE__,
					 tn_keyboard[kbd->new[i]],
					 tn_keyboard[kbd->new[i]]);
			} else
				kb_debug(
	...
	for (i = 0; i < 2; i++) {
		...
		if (memscan_ex(kbd->mm_new, keywords, 4) == kbd->mm_new + 4) {
			if (keywords) {
				...
				kb_debug(
					"%s %d i:%d j:%d %p!=%p keywords:0x%x\n",
					__func__, __LINE__, i, j,
					memscan_ex(kbd->mm_new, keywords, 4),
					kbd->mm_new + 4, keywords);
				if (j < KEYBOARD_MM_NUM_KEYS) {
					input_event(input_dev, EV_MSC, MSC_SCAN,
						    (0x0c << 16) | keywords);
					kb_debug(
						"%s %d input_event MSC_SCAN:0x%x \n",
						__func__, __LINE__,
						(0x0c << 16) | keywords);
					input_report_key(input_dev,
							 mm_tn_keyboard[j][1],
							 0);
					kb_debug(
						"%s %d key:0x%x scancode:0x%x\n",
						__func__, __LINE__, keywords,
						mm_tn_keyboard[j][1]);
					//tn_keyboard_led_report(mm_tn_keyboard[j][1]);
				} else {
					kb_debug(
						"Unknown key (scancode %#x) %d released.\n",
		...
		if (memscan_ex(kbd->mm_old, keywords, 4) == kbd->mm_old + 4) {
			if (keywords) {
				...
				kb_debug(
					"%s %d i:%d j:%d %p!=%p keywords:0x%x\n",
					__func__, __LINE__, i, j,
					memscan_ex(kbd->mm_old, keywords, 4),
					kbd->mm_new + 4, keywords);
				if (j < KEYBOARD_MM_NUM_KEYS) {
					input_event(input_dev, EV_MSC, MSC_SCAN,
						    (0x0c << 16) | keywords);
					...
					kb_debug(
						"Unknown key (scancode %#x) %d  pressed.\n",
