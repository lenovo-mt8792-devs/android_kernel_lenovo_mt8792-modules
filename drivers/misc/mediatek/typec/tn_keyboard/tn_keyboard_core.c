...
void set_wakeup_irq_switch(int enable)
{
	...
	if (!tn_keyboard_client->uart_wake_gpio_irq) {
		kb_debug(
			"set_wakeup_irq_switch uart_wake_gpio_irq == NULL!cancel!\n");
		return;
	}
	wake_irq_switch = enable;
	if (enable == 1) {
		...
		kb_debug(
			"set_wakeup_irq_switch enable_irq uart_wake_gpio_irq!\n");
	} else if (enable == 0) {
		tn_keyboard_client->uart_wake_gpio_irq_count = 3;
		disable_irq(tn_keyboard_client->uart_wake_gpio_irq);
		kb_debug(
			"set_wakeup_irq_switch disable_irq uart_wake_gpio_irq!\n");
	}
}
...
static int tn_keyboard_adc_init(void)
{
	...
	int val = 0;
	kb_debug("%s %d start\n", __func__, __LINE__);
	tn_keyboard_client->adc_check_ch =
		devm_kzalloc(&tn_keyboard_client->plat_dev->dev,
			     sizeof(struct iio_channel), GFP_KERNEL);
	if (!tn_keyboard_client->adc_check_ch)
		return -ENOMEM;
	tn_keyboard_client->adc_check_ch = iio_channel_get(
		&tn_keyboard_client->plat_dev->dev, "tn_keyboard-ch3");
	...
	ret = iio_read_channel_processed(tn_keyboard_client->adc_check_ch,
					 &val);
...
void tn_keyboard_start_tp_down_loss_check_timer(int enable)
{
	if (tn_keyboard_client->tp_down_loss_timer_enable == 0 && enable) {
		...
		alarm_start_relative(
			&tn_keyboard_client->tp_down_loss_check_timer,
			ms_to_ktime(850));
...
{
	if (tn_keyboard_client->pad_hall_delay_close_vcc_enable == 0 &&
	    enable) {
		alarm_start_relative(
			&tn_keyboard_client->pad_hall_delay_close_vcc_timer,
			ms_to_ktime(60 * 1000));
...
static int tn_keyboard_mod_data_process(char *buf, int len)
{
	...
	if (tn_keyboard_client->vcc_on == 1 && value != 0x7b &&
	    (tn_keyboard_client->tn_keyboard_status &
	     KEYBOARD_CONNECT_STATUS) == 0) {
	...
	case 0x62:
		...
		break;
	case 0x63:
		if ((tn_keyboard_client->tn_keyboard_status &
		     KEYBOARD_INPUT_INIT_STATUS) != 0) {
			kb_debug("%s %d	touchpad_input_report begin!\n",
				 __func__, __LINE__);
			touchpad_input_report(&buf[1]);
		} else {
			kb_debug(
				"%s %d	cancel touchpad_input_report!keyboard no INPUT INIT!status: %d vcc_on:%d!\n",
				__func__, __LINE__,
				tn_keyboard_client->tn_keyboard_status,
				tn_keyboard_client->vcc_on);
		}
		break;
	case 0x7b:
		...
		if (buf[2] == 0x02 && len > 2) {
			kb_debug(
				"%s %d start tn_keyboard_start_tp_down_loss_check_timer\n",
				__func__, __LINE__);
	...
	case 0x92:
		if (tn_keyboard_client->read_flag == 1) {
			kb_debug("%s %d	value:0x%2x cmd:0x%2x \n", __func__,
				 __LINE__, value,
				 tn_keyboard_client->write_buf[1]);
			if (value == (int)tn_keyboard_client->write_buf[1] ||
			    value ==
				    (int)tn_keyboard_client->write_buf[1] + 1) {
...
int uart_unpack_date(char *buf, int len, unsigned char *out_buf, int *out_len)
{
	...
	f_dec = one_wire_bus_decode_frame(&temp[2],
...
int tn_keyboard_analyze(char *buf, int len)
{
	...
	for (i = 0; i < len; i++) {
		...
		if (pbuf[i] == ONE_WIRE_BUS_PACKET_START_FLAG) {
			...
			if (tn_keyboard_client->recv_status ==
			    KEYBOARD_UART_RECV_DEFAULT)
				tn_keyboard_client->recv_status =
		...
		if (tn_keyboard_client->recv_len < UART_BUFFER_SIZE &&
	...
	if (tn_keyboard_client->recv_status != KEYBOARD_UART_RECV_END) {
		if (tn_keyboard_client->recv_status == KEYBOARD_UART_RECV_START)
			kb_info("%s %d revc data is not KEYBOARD_UART_RECV_END!recv data: ",
				__func__, __LINE__);
		else {
			kb_err("%s %d recv exception data fail!len:%d recv_status:%d recv data: ",
			       __func__, __LINE__, len,
			       tn_keyboard_client->recv_status);
		}
	...
	if (ret) {
		sprintf(TAG, "%s  %d uart_unpack_date fail!recv_count:%d ",
...
ssize_t tty_write_ex(const char *buf, size_t count)
{
	...
	if (filp != tn_keyboard_client->file_client) {
		kb_err("%s %d replace file_client %p to %p!\n", __func__,
		       __LINE__, tn_keyboard_client->file_client, filp);
...
int tn_keyboard_get_valid_data(char *buf, int len, unsigned char *out_buf,
			       int *out_len)
{
	char *pbuf = buf;
	int i = 0;
...
static int tn_keyboard_write(void *buf, int len)
{
	...
	memset(tn_keyboard_client->write_buf, 0,
	       sizeof(tn_keyboard_client->write_buf));
	...
	tn_keyboard_get_valid_data(write_buf, write_len,
				   tn_keyboard_client->write_buf, &out_len);
	for (j = 0; j <= count; j++) {
		for (i = 0; i < count; i++) {
		...
		wait_event_interruptible_timeout(
			read_waiter, tn_keyboard_client->read_flag != 1,
			HZ / 20); //timeout 50ms

		if (tn_keyboard_client->read_flag == 1) {
			kb_debug("%s %d read_flag:%d  \r\n", __func__, __LINE__,
				 tn_keyboard_client->read_flag);
		}
		if (tn_keyboard_client->write_len <= 0) {
			kb_err("%s %d read_flag:%d  write_len:%d\r\n", __func__,
			       __LINE__, tn_keyboard_client->read_flag,
			       tn_keyboard_client->write_len);
			tn_keyboard_client->read_flag = 0;
			return -1;
		}
		if (tn_keyboard_client->write_buf[1] ==
			    tn_keyboard_client->write_check_buf[1] &&
		    tn_keyboard_client->write_buf[out_len - 2] ==
			    tn_keyboard_client->write_check_buf
				    [out_len - 2]) { //compare cmd and crc
			ret = 0;
			kb_debug(
				"%s %d is same,write %d data ok ret:%d i:%d!\n",
...
static int tn_keyboard_read(void *buf, int *r_len)
{
	...
	if (tn_keyboard_client->read_len <= 0) {
		kb_err("%s %d read_len:%d  err\r\n", __func__, __LINE__,
		       tn_keyboard_client->read_len);
		tn_keyboard_client->read_flag = 0;
		return -1;
	}
	kb_debug("%s %d read_flag:%d len:%d ok\r\n", __func__, __LINE__,
		 tn_keyboard_client->read_flag, tn_keyboard_client->read_len);
	ret = uart_unpack_date(tn_keyboard_client->read_buf,
			       tn_keyboard_client->read_len, buf, r_len);
	memset(tn_keyboard_client->read_buf, 0,
	       sizeof(tn_keyboard_client->read_buf));
	sprintf(TAG, "%s text %d", __func__, __LINE__);
...
static int tn_keyboard_write_and_read(void *w_buf, int w_len, void *r_buf,
				      int *r_len)
{
	int ret = 0;
	int i = 0;
	...
	for (i = 0; i < count; i++) {
		kb_debug("%s %d read_flag:%d i:%d start\r\n", __func__,
			 __LINE__, tn_keyboard_client->read_flag, i);
...
int tn_keyboard_tp_ver(void)
{
	...
	ret = tn_keyboard_write_and_read(ver_reg, sizeof(ver_reg), temp,
					 &read_len);
	if (ret < 0) {
...
static int tn_keyboard_set_power(int power_status)
{
	...
	for (i = 0; i < 3; i++) {
		ret = tn_keyboard_write_and_read(power_buf, sizeof(power_buf),
						 read_buf, &read_len);
		if (ret) {
			continue;
		}
...
{
	char value = 0;
	if (!(key_value == KEY_CAPSLOCK || key_value == KEY_MUTE ||
	      key_value == KEY_MICDISABLE))
		return;
	kb_debug("%s %d key_value:%d\r\n", __func__, __LINE__, key_value);
	if (key_value == KEY_CAPSLOCK) {
		if ((tn_keyboard_client->tn_keyboard_status &
		     KEYBOARD_CAPSLOCK_ON_STATUS) == 0) {
			tn_keyboard_client->tn_keyboard_status |=
				KEYBOARD_CAPSLOCK_ON_STATUS;
		} else {
			tn_keyboard_client->tn_keyboard_status &=
		...
		if ((tn_keyboard_client->tn_keyboard_status &
		     KEYBOARD_MUTEDISABLE_ON_STATUS) == 0) {
			tn_keyboard_client->tn_keyboard_status |=
				KEYBOARD_MUTEDISABLE_ON_STATUS;
		} else {
			tn_keyboard_client->tn_keyboard_status &=
				(~KEYBOARD_MUTEDISABLE_ON_STATUS);
		}
		value = tn_keyboard_client->tn_keyboard_status &
			KEYBOARD_MUTEDISABLE_ON_STATUS;
		//input_event(tn_keyboard_client->input_tn_keyboard, EV_LED, LED_MUTE, !!value);
		kb_debug("%s %d KEY_MUTE key_value:%d value:%d\r\n", __func__,
			 __LINE__, key_value, value);

	} else if (key_value == KEY_MICDISABLE) {
		if ((tn_keyboard_client->tn_keyboard_status &
		     KEYBOARD_MICDISABLE_ON_STATUS) == 0) {
			tn_keyboard_client->tn_keyboard_status |=
				KEYBOARD_MICDISABLE_ON_STATUS;
		} else {
			tn_keyboard_client->tn_keyboard_status &=
				(~KEYBOARD_MICDISABLE_ON_STATUS);
		}
...
void tn_keyboard_led_process(int code, int value)
{
	...
	default:
		kb_debug(
			"%s %d  type:led code:0x%2x value:0x%2x no support err!\n",
			__func__, __LINE__, code, value);
		return;
	}
...
static int tn_keyboard_notifier_callback(struct notifier_block *nb,
...
int tn_keyboard_enable_uart_tx(int value)
{
	...
	kb_debug("%s %d  value:%d gpiovalue:%d\n", __func__, __LINE__, value,
		 gpio_get_value_cansleep(tn_keyboard_client->tx_en_gpio));
	if (value == 1) {
#ifdef CONFIG_BOARD_V4_SUPPORT
		...
		gpio_set_value_cansleep(tn_keyboard_client->tx_en_gpio,
					1); //bygx
		kb_debug("%s %d  set 1\n", __func__, __LINE__);
		//}
...
#else
		pinctrl_select_state(tn_keyboard_client->pinctrl,
				     tn_keyboard_client->uart_rx_clear);
		pinctrl_select_state(tn_keyboard_client->pinctrl,
				     tn_keyboard_client->uart_tx_set);
		udelay(450); //for set mode valid
#endif
	} else if (value == 0) {
#ifdef CONFIG_BOARD_V4_SUPPORT
		...
		udelay(300); //for set mode valid
		pinctrl_select_state(tn_keyboard_client->pinctrl,
				     tn_keyboard_client->uart_tx_clear);
		pinctrl_select_state(tn_keyboard_client->pinctrl,
				     tn_keyboard_client->uart_rx_set);
#endif
	}
	return 0;
}
...
int tn_keyboard_get_dts_info(void)
{
	...
	tn_keyboard_client->plug_gpio =
		of_get_named_gpio(node, "plug-gpios", 0);
	...
	ret = gpio_request_one(tn_keyboard_client->plug_gpio, GPIOF_DIR_IN,
			       "keybaord_plug_gpio");
	if (ret) {
	...
	tn_keyboard_client->plug_irq =
		gpio_to_irq(tn_keyboard_client->plug_gpio);
	ret = devm_request_threaded_irq(&device->dev,
					tn_keyboard_client->plug_irq,
					keybaord_core_plug_irq_handler, NULL,
					IRQF_TRIGGER_FALLING,
					"keybaord_plug_irq", NULL);
	if (ret < 0) {
	...
	tn_keyboard_client->power_en_gpio =
		of_get_named_gpio(node, "power-en-gpios", 0);
	...
	tn_keyboard_client->uart_tx_gpio =
		of_get_named_gpio(node, "uart-tx-gpios", 0);
	...
	tn_keyboard_client->uart_rx_gpio =
		of_get_named_gpio(node, "uart-rx-gpios", 0);
	...
	tn_keyboard_client->pinctrl =
		devm_pinctrl_get(&tn_keyboard_client->plat_dev->dev);
	if (IS_ERR(tn_keyboard_client->pinctrl)) {
		kb_err("%s %d devm_pinctrl_get  err\n", __func__, __LINE__);
		return -1;
	}
	tn_keyboard_client->uart_tx_set = pinctrl_lookup_state(
		tn_keyboard_client->pinctrl, "uart_tx_set");
	if (IS_ERR(tn_keyboard_client->uart_tx_set)) {
		kb_debug("%s %d pinctrl_lookup_state uart_tx_set err\n",
			 __func__, __LINE__);
		return -1;
	}
	tn_keyboard_client->uart_tx_clear = pinctrl_lookup_state(
		tn_keyboard_client->pinctrl, "uart_tx_clear");
	...
	tn_keyboard_client->uart_rx_set = pinctrl_lookup_state(
		tn_keyboard_client->pinctrl, "uart_rx_set");
	if (IS_ERR(tn_keyboard_client->uart_rx_set)) {
		kb_err("%s %d pinctrl_lookup_state uart_rx_set err\n", __func__,
		       __LINE__);
		return -1;
	}
	tn_keyboard_client->uart_rx_clear = pinctrl_lookup_state(
		tn_keyboard_client->pinctrl, "uart_rx_clear");
	if (IS_ERR(tn_keyboard_client->uart_rx_clear)) {
		kb_err("%s %d pinctrl_lookup_state uart_rx_clear err\n",
		       __func__, __LINE__);
		return -1;
	}
	tn_keyboard_client->uart_rx_gpio_pin = pinctrl_lookup_state(
		tn_keyboard_client->pinctrl, "uart_rx_gpio");
	...
	pinctrl_select_state(tn_keyboard_client->pinctrl,
			     tn_keyboard_client->uart_rx_gpio_pin);
	ret = gpio_request_one(tn_keyboard_client->uart_rx_gpio, GPIOF_DIR_IN,
			       "tn_keyboard_rx_gpio");
	if (ret) {
	...
	tn_keyboard_client->uart_rx_gpio_irq =
		gpio_to_irq(tn_keyboard_client->uart_rx_gpio);
	...
	tn_keyboard_client->uart_wake_gpio =
		of_get_named_gpio(node, "uart-wake-gpios", 0);
	...
	tn_keyboard_client->tx_en_gpio =
		of_get_named_gpio(node, "tx-en-gpios", 0);
	...
	kb_debug(
		"%s %d uart_wake_gpio:%d tx_en_gpio:%d uart_wake_gpio_irq:%d\n",
...
static ssize_t tx_mode_store(struct device *dev, struct device_attribute *attr,
...
static ssize_t tx_mode_show(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
	return 0;
}
...
static DEVICE_ATTR(test_mode, S_IRUGO | S_IWUSR, test_mode_show,
		   test_mode_store);
static DEVICE_ATTR(keyboard_status, S_IRUGO | S_IWUSR, keyboard_status_show,
		   NULL);
static DEVICE_ATTR(hall_status, S_IRUGO | S_IWUSR, hall_status_show, NULL);

/* add your attr in here*/
static struct attribute *tn_keyboard_attributes[] = {
	&dev_attr_tx_mode.attr, &dev_attr_test_mode.attr,
	&dev_attr_keyboard_status.attr, &dev_attr_hall_status.attr, NULL
};
...
static int tn_keyboard_event_process(unsigned char tn_keyboard_event)
{
	...
	kb_debug(
		"%s %d BEGIN tn_keyboard_event:%d  tn_keyboard_status:0x%02x\n",
		__func__, __LINE__, tn_keyboard_event,
		tn_keyboard_client->tn_keyboard_status);
	switch (tn_keyboard_event) {
	case KEYBOARD_PLUG_IN_EVENT:
		hrtimer_cancel(&tn_keyboard_client->plug_timer);
		tn_keyboard_client->tn_keyboard_status |=
			KEYBOARD_CONNECT_STATUS;
	...
	case KEYBOARD_WAKEUP_IRQ_EVENT:
		if (!IS_ERR(sec_device)) {
			...
			kb_debug(
				"%s %d KEYBOARD_WAKEUP_IRQ_EVENT KOBJ_ADD ERROR!\n",
				__func__, __LINE__);
		}
	...
	case KEYBOARD_PLUG_VCC_EVENT:
		...
		if (value == 0) {
			...
			if (!IS_ERR(sec_device)) {
				kb_debug(
					"%s %d KEYBOARD_PLUG_VCC_EVENT KOBJ_ADD!\n",
					__func__, __LINE__);
				kobject_uevent(&sec_device->kobj, KOBJ_ADD);
			} else {
				kb_debug(
					"%s %d KEYBOARD_PLUG_VCC_EVENT KOBJ_ADD ERROR!\n",
					__func__, __LINE__);
			}
			if (tn_keyboard_client->vcc_on == 1) {
				kb_debug(
					"%s %d KEYBOARD_PLUG_VCC_EVENT END!tn_keyboard_client->vcc_on == 1\n",
					__func__, __LINE__);
				goto vcc_end;
			}
			tn_keyboard_client->vcc_on = 1;
			irq_set_irq_type(tn_keyboard_client->plug_irq,
					 IRQF_TRIGGER_RISING);
#ifdef CONFIG_ADC_SUPPORT
			tn_keyboard_adc_get();
			...
			if (!tn_keyboard_client->uart_wake_gpio_irq &&
			    tn_keyboard_client->plat_dev != NULL) {
				...
				if (ret) {
					...
					ret = devm_request_threaded_irq(
						&tn_keyboard_client->plat_dev
							 ->dev,
						tn_keyboard_client
							->uart_wake_gpio_irq,
						tn_keyboard_wake_gpio_irq_handler,
						NULL,
						IRQF_TRIGGER_RISING |
							IRQF_ONESHOT,
						"keybaord_wake_irq", NULL);
					if (ret < 0) {
						kb_err("request irq failed : %d\n",
						       tn_keyboard_client
							       ->uart_wake_gpio_irq);
...
vcc_end:
			pm_relax(&tn_keyboard_client->plat_dev->dev);
			kb_debug(
				"%s %d KEYBOARD_PLUG_VCC_EVENT END!vcc_on:%d\n",
	...
	case KEYBOARD_PLUG_OUT_EVENT:
		...
		if (!IS_ERR(sec_device)) {
			...
			kb_debug(
				"%s %d KEYBOARD_PLUG_OUT_EVENT KOBJ_REMOVE ERROR!\n",
				__func__, __LINE__);
		}
	...
	case KEYBOARD_HOST_LCD_ON_EVENT:
		if (tn_keyboard_client->vcc_on == 1) {
			if (gpio_get_value(tn_keyboard_client->power_en_gpio) ==
			    0) {
			...
			if (!IS_ERR(sec_device)) {
				kb_debug(
					"%s %d KEYBOARD_HOST_LCD_ON_EVENT KOBJ_ADD!\n",
					__func__, __LINE__);
				kobject_uevent(&sec_device->kobj, KOBJ_ADD);
			}
		}
		...
		if ((tn_keyboard_client->tn_keyboard_status &
			...
			if (!IS_ERR(sec_device)) {
				...
				kb_debug(
					"%s %d KEYBOARD_HOST_LCD_ON_EVENT KOBJ_ADD ERROR!\n",
					__func__, __LINE__);
			}
	...
	case KEYBOARD_HOST_LCD_OFF_EVENT:
		...
		if ((tn_keyboard_client->tn_keyboard_status &
			...
			if (!IS_ERR(sec_device)) {
				...
				kb_debug(
					"%s %d KEYBOARD_HOST_LCD_OFF_EVENT KOBJ_REMOVE ERROR!\n",
					__func__, __LINE__);
			}
...
struct file *tn_keyboard_port_to_file(struct uart_8250_port *uart_8250_port)
{
	...
	list_for_each_entry(priv, &tty->tty_files, list) {
		if (priv != NULL) {
			kb_debug(
				"%s %d  major:%d minor:%d found!ttyS majodr=4 minor=65 priv->file:%p",
				__func__, __LINE__, imajor(priv->file->f_inode),
				iminor(priv->file->f_inode), priv->file);
			if (filp == NULL &&
			    (imajor(priv->file->f_inode) == 4 ||
			     iminor(priv->file->f_inode) == 65)) {
				filp = priv->file;
				kb_debug("%s %d  %p found!file_client:%p",
					 __func__, __LINE__, priv->file,
					 tn_keyboard_client->file_client);
				//break;
			}
			if (tn_keyboard_client->file_client != NULL &&
			    tn_keyboard_client->file_client == priv->file) {
				filp = tn_keyboard_client->file_client;
				kb_debug(
					"%s %d priv->file:%p == file_client:%p",
					__func__, __LINE__, priv->file,
					tn_keyboard_client->file_client);
			}
		}
...
int tn_keyboard_dma_set_call_back(void *uart_8250_port, int type)
{
	struct file *filp;
	if (type == 1) {
		tn_keyboard_client->port =
			(struct uart_8250_port *)uart_8250_port;
		filp = tn_keyboard_port_to_file(tn_keyboard_client->port);
...
static void tn_keyboard_pad_hall_status_call_back(int status)
{
	...
	if (status == PAD_HALL_STATU_CLOSE) {
		...
		kb_err("%s %d pad hall open not handle vcc!vcc_hall:%d vcc_gpio_value:%d:!\n",
		       __func__, __LINE__, tn_keyboard_client->vcc_on,
		       gpio_get_value(tn_keyboard_client->power_en_gpio));
	}
}
...
static void tn_keyboard_register_call_back(void)
{
	...
	if (pad_hall_status_call_back == NULL) {
		pad_hall_status_call_back =
			tn_keyboard_pad_hall_status_call_back;
	}
}
...
static enum hrtimer_restart keybaord_core_plug_hrtimer(struct hrtimer *timer)
{
	...
	if (value == 0) {
		...
		kb_debug("%s %d %d\n", __func__, __LINE__, value);
		ktime = ktime_set(1, 0);
		hrtimer_start(&tn_keyboard_client->plug_timer, ktime,
			      HRTIMER_MODE_REL);
...
keybaord_core_tp_down_loss_hrtimer(struct alarm *alarm, ktime_t now)
{
...
static enum alarmtimer_restart
pad_hall_delay_close_vcc_timer(struct alarm *alarm, ktime_t now)
{
...
static int tn_keyboard_plat_probe(struct platform_device *device)
{
	...
	tn_keyboard_client->tn_keyboard_notify.notifier_call =
		tn_keyboard_notifier_callback;
	ret = mtk_disp_notifier_register(
		"tn_keyboard", &tn_keyboard_client->tn_keyboard_notify);
	...
	tn_keyboard_client->tn_keyboard_task =
		kthread_run(tn_keyboard_event_handler, 0, "tn_keyboard_task");
	...
	ret = sysfs_create_group(&device->dev.kobj,
				 &tn_keyboard_attribute_group);
	if (ret != 0) {
	...
	hrtimer_init(&tn_keyboard_client->plug_timer, CLOCK_MONOTONIC,
		     HRTIMER_MODE_REL);
	...
	alarm_init(&tn_keyboard_client->tp_down_loss_check_timer,
		   ALARM_BOOTTIME, keybaord_core_tp_down_loss_hrtimer);
	alarm_init(&tn_keyboard_client->pad_hall_delay_close_vcc_timer,
		   ALARM_BOOTTIME, pad_hall_delay_close_vcc_timer);
...
static int tn_keyboard_plat_suspend(struct platform_device *device,
				    pm_message_t state)
{
...
static struct platform_driver
	tn_keyboard_plat_driver = { .probe = tn_keyboard_plat_probe,
...
static int __init tn_keyboard_mod_init(void)
{
	...
	if (IS_ERR(sec_class)) {
		...
		sec_device =
			device_create(sec_class, NULL, 0, NULL, "keyboard");
		if (IS_ERR(sec_device)) {
