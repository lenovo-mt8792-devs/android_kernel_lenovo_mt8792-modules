...
int touchpad_input_report(char *buf)
{
	...
	if (tn_keyboard_client->pre_fingers != fingers) {
		kb_debug("%s %d finger change!flingers:%d -> %d!\n", __func__,
			 __LINE__, tn_keyboard_client->pre_fingers, fingers);
		if (tn_keyboard_client->touch_down > 0) {
			...
			if (unlikely(tn_keyboard_client->touch_down ^
				     temp_touch_down)) {
				kb_debug(
					"%s %d auto report finger up!touch_down:%d temp_touch_down:%d\n",
					__func__, __LINE__,
					tn_keyboard_client->touch_down,
					temp_touch_down);
				for (i = 0; i < TOUCH_FINGER_MAX; i++) {
					if (BIT(i) &
					    (tn_keyboard_client->touch_down ^
					     temp_touch_down)) { //finger change
						input_mt_slot(input_dev, i);
						input_mt_report_slot_state(
							input_dev,
							MT_TOOL_FINGER, false);
						tn_keyboard_client->pre_fingers =
							fingers;
						tn_keyboard_client->touch_down &=
							~BIT(i);
						tn_keyboard_client->touch_temp &=
							~BIT(i);
						kb_debug(
							"%s %d auto report finger up!change id:%d finger change:%d -> %d touch_down:%d temp_touch_down:%d\n",
							__func__, __LINE__, i,
							fingers,
							tn_keyboard_client
								->pre_fingers,
			...
			kb_debug(
				"%s %d auto report finger up ok!touch_down:%d temp_touch_down:%d\n",
				__func__, __LINE__,
				tn_keyboard_client->touch_down,
				temp_touch_down);
		}
	...
	for (j = 0; j < fingers; j++) {
		...
		kb_debug(
			"%s %d id:%d,down:%d,left:%d,right:%d,x:%d,y:%d,area:%d offset:%d fingers:%d\n",
			__func__, __LINE__, temp.id, temp.is_down, temp.is_left,
			temp.is_right, temp.x, temp.y, temp.area, offset,
			fingers);
		...
		if (temp.is_down) {
				...
				for (i = 0; i < TOUCH_FINGER_MAX; i++) {
					if (BIT(i) &
						...
						tn_keyboard_client
							->pre_fingers -= 1;
						kb_debug(
							"%s %d check finger up!change id:%d finger change:%d -> %d touch_down:%d touch_temp:%d\n",
							__func__, __LINE__, i,
							fingers,
							tn_keyboard_client
								->pre_fingers,
			...
			kb_debug(
				"%s %d finger up!id:%d touch_down:%d touch_temp:%d\n",
			...
			if (!tn_keyboard_client->touch_down) { //finger all up
				...
				kb_debug(
					"%s %d finger all up!clear pre_fingers:%d\n",
					__func__, __LINE__,
					tn_keyboard_client->pre_fingers);
			}
		}
		if (j == fingers - 1) {
			if (temp.is_left) {
				...
				input_report_key(input_dev, BTN_MOUSE,
						 temp.is_left);
				kb_debug(
					"%s %d report touch left BTN_MOUSE down!\n",
					__func__, __LINE__);
			}
			if (temp.is_right) {
				...
				input_report_key(input_dev, BTN_RIGHT,
						 temp.is_right);
				kb_debug(
					"%s %d report touch right BTN_RIGHT down!\n",
					__func__, __LINE__);
			}
			if ((tn_keyboard_client->tn_keyboard_status &
			     KEYBOARD_POWER_ON_STATUS) == 0 &&
			    event->is_left == 0 && event->is_right == 0 &&
			    tn_keyboard_client->tp_down_loss_timer_enable ==
				    0) {
				kb_debug(
					"%s %d tn_keyboard_start_tp_down_loss_check_timer!\n",
					__func__, __LINE__);
			...
			if (event->is_left && temp.is_left == 0) {
				...
				input_report_key(input_dev, BTN_MOUSE,
						 temp.is_left);
				kb_debug(
					"%s %d report touch left BTN_MOUSE up!\n",
					__func__, __LINE__);
			}
			if (event->is_right && temp.is_right == 0) {
				...
				input_report_key(input_dev, BTN_RIGHT,
						 temp.is_right);
				kb_debug(
					"%s %d report touch right BTN_RIGHT up!\n",
					__func__, __LINE__);
			}
		}
...
int touchpad_input_init(void)
{
	...
	input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0, TOUCH_X_MAX, 0,
			     0);
	input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0, TOUCH_Y_MAX, 0,
			     0);
