...
void i2c_table_write(struct subdrv_ctx *ctx, u16 *list, u32 len)
{
    ...
    case I2C_DT_ADDR_16_DATA_16:
        ...
        break;
    case I2C_DT_ADDR_16_DATA_8_SEPAR:
        for (int i = 0; i < len;) {
            subdrv_i2c_wr_u8(ctx, list[i], list[i+1]);
            i += 2;
        }
        break;
...
void streaming_control(struct subdrv_ctx *ctx, bool enable)
{
	...
	if (enable) {
		...
		if(ctx->s_ctx.sensor_id == SC820CS_SENSOR_ID)
		{
		    mdelay(10);
		}
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		do {
			*sensor_id = (subdrv_i2c_rd_u8(ctx, addr_h) << 8) |
			...
			pr_err("i2c_write_id:0x%x sensor_id(cur/exp):0x%x/0x%x\n",
