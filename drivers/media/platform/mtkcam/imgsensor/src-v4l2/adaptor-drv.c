...
static int search_sensor(struct adaptor_ctx *ctx)
{
	...
	for (i = 0; i < subdrvs_cnt; i++) {
		...
		if (!ret) {
			...
			if(!(strcmp("gc13a0_mipi_raw", (char*)ctx->subdrv->name))) {
				hardwareinfo_set_prop(HARDWARE_BACK_CAM, "One_Back_LCE_Gc13a0_mipi_raw");
				hardwareinfo_set_prop(HARDWARE_BACK_CAM_MOUDULE_ID, "LCE");
			} else if (!(strcmp("ov13b10_mipi_raw", (char*)ctx->subdrv->name))) {
				hardwareinfo_set_prop(HARDWARE_BACK_CAM, "Two_Back_TXD_Ov13b10_mipi_raw");
				hardwareinfo_set_prop(HARDWARE_BACK_CAM_MOUDULE_ID, "TXD");
			}

			if(!(strcmp("sc820cs_mipi_raw", (char*)ctx->subdrv->name))) {
				hardwareinfo_set_prop(HARDWARE_FRONT_CAM, "One_Front_TXD_Sc820cs_mipi_raw");
				hardwareinfo_set_prop(HARDWARE_FRONT_CAM_MOUDULE_ID, "TXD");
			} else if (!(strcmp("s5k4h7_mipi_raw", (char*)ctx->subdrv->name))) {
				hardwareinfo_set_prop(HARDWARE_FRONT_CAM, "Two_Front_LCE_S5k4h7_mipi_raw");
